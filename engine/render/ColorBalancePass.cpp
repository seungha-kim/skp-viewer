...
public:
    ...
    ColorBalancePassOutput render(RenderContext& ctx, const ColorBalancePassInput& input) {
        ...
        m_textureRenderer.render(ctx, [&](Shader& shader) { shader.setVector3f("colorBalance", m_colorBalance); });
        return {
...
ColorBalancePass::ColorBalancePass(const SurfaceInfo& surfaceInfo)
        : m_pimpl(std::make_unique<ColorBalancePassPimpl>(surfaceInfo)) { }
