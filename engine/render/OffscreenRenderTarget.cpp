...
void OffscreenRenderTarget::setTargetColorTexture(const ColorTexture& texture, int index) {
    ...
    auto binding = bind();
    glBindFramebuffer(GL_FRAMEBUFFER, m_fbo);
    glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + index, texture.textureName(), 0);
}
...
void OffscreenRenderTarget::setTargetDepthTexture(const DepthTexture& texture) {
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, m_fbo);
    glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, texture.textureName(), 0);
}
